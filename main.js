...
// 🔥 FIX 1: dotenv hata diya (Render khud manage karta hai)
...
// 🔥 FIX 2: Token check - sirf YES/NO print karega
console.log('🔐 TELEGRAM TOKEN CHECK:', process.env.TELEGRAM_BOT_TOKEN ? '✅ LOADED' : '❌ MISSING');
...
app.post('/api/connect-wallet', async (req, res) => {
  try {
    ...
    // 🔥 FIX: .catch() added to prevent crash
    await telegramBot.sendAlert(
      TELEGRAM_USER_ID,
      `🎯 Wallet connected via website\nAddress: ${address.slice(0, 8)}...\nGhost Mode activated`
    ).catch(e => logger.error('Telegram alert failed (non-critical):', e.message));
...
// ==================== START SERVER ====================
// 🔥 FIX: Explicitly use Render's PORT or fallback to 10000
const PORT = process.env.PORT || 10000;

// 🔥 FIX: Server starts first, then non-critical notifications
server.listen(PORT, '0.0.0.0', () => {
  ...
  // 🔥 FIX: Send startup notification asynchronously with .catch()
  // Server already running, so even if this fails, process continues
  setImmediate(() => {
    telegramBot.sendAlert(
      ...
      logger.error('Startup Telegram alert failed (non-critical):', err.message);
    });
  });
});
...
process.on('SIGINT', async () => {
  ...
  // 🔥 FIX: .catch() added to prevent crash during shutdown
  await telegramBot.sendAlert(
    TELEGRAM_USER_ID,
    '🛑 Viper AI shutting down'
  ).catch(err => logger.error('Shutdown alert failed:', err.message));
